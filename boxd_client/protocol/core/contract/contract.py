...
from boxd_client.protocol.core.contract.blocks import (
    is_hex_encoded_block_hash,
)
from boxd_client.protocol.core.contract.contracts import (
    encode_abi,
...
from boxd_client.protocol.core.contract.datatypes import (
    PropertyCheckingFactory,
)
from boxd_client.protocol.core.contract.decorators import (
    combomethod,
    deprecated_for,
)
from boxd_client.protocol.core.contract.empty import (
    empty,
)
from boxd_client.protocol.core.contract.encoding import (
    to_4byte_hex,
    to_hex,
)
from boxd_client.protocol.core.contract.events import (
    EventFilterBuilder,
    get_event_data,
    is_dynamic_sized_type,
)
from boxd_client.protocol.core.contract.filters import (
    construct_event_filter_params,
)
from boxd_client.protocol.core.contract.function_identifiers import (
    FallbackFn,
)
...
from boxd_client.protocol.core.contract.exceptions import (
...
class ContractEvents:
    ...
    .. code-block:: python
        ...
        >>> mycontract.events
        <boxd.contract.ContractEvents object at 0x108afde10>
    ...
    .. code-block:: python
        ...
        <class 'boxd._utils.datatypes.LogAnonymous'>
        ...
...
class Contract:
    ...
    * Deploy a new smart contract using :py:meth:`Contract.deploy`
    ...
    def encodeABI(cls, fn_name, args=None, kwargs=None, data=None):
        ...
        return encode_abi(cls.boxd, fn_abi, fn_arguments, data)

    @combomethod
    def all_functions(self):
        return find_functions_by_identifier(
            self.abi, self.boxd, self.address, lambda _: True
        )
    ...
    def get_function_by_signature(self, signature):
        ...
        fns = find_functions_by_identifier(self.abi, self.boxd, self.address, callable_check)
    ...
    def find_functions_by_name(self, fn_name):
        ...
        return find_functions_by_identifier(
            self.abi, self.boxd, self.address, callable_check
        )
    ...
    @staticmethod
    def get_fallback_function(abi, boxd, address=None):
    ...
    def _encode_constructor_data(cls, args=None, kwargs=None):
        ...
        if constructor_abi:
            ...
            deploy_data = add_0x_prefix(
                encode_abi(cls.boxd, constructor_abi, arguments, data=cls.bytecode)
            )
        else:
...
class ContractConstructor:
    ...
    def _encode_data_in_transaction(self, *args, **kwargs):
        ...
        if constructor_abi:
            ...
            arguments = merge_args_and_kwargs(constructor_abi, args, kwargs)
            data = add_0x_prefix(
                encode_abi(self.boxd, constructor_abi, arguments, data=self.bytecode)
            )
        else:
    ...
    def estimateGas(self, transaction=None):
        ...
        if self.boxd.eth.defaultAccount is not empty:
            estimate_gas_transaction.setdefault('from', self.boxd.eth.defaultAccount)
    ...
    def buildTransaction(self, transaction=None):
        ...
        if self.boxd.eth.defaultAccount is not empty:
            built_transaction.setdefault('from', self.boxd.eth.defaultAccount)

        built_transaction['data'] = self.data_in_transaction
        built_transaction['to'] = b''
        return fill_transaction_defaults(self.boxd, built_transaction)
...
class ContractFunction:
    ...
    def _encode_transaction_data(cls):
        return add_0x_prefix(encode_abi(cls.boxd, cls.abi, cls.arguments, cls.selector))
...
class ContractEvent:
    ...
    def createFilter(
        ...
        log_filter = filter_builder.deploy(self.boxd)
    ...
    def getLogs(self,
        ...
        # Call JSON-RPC API
        logs = self.boxd.eth.getLogs(event_filter_params)
...
def call_contract_function(
    ...
    try:
        ...
        is_missing_code_error = (
            ...
            boxd.eth.getCode(address) in ACCEPTABLE_EMPTY_STRINGS
        )
...
def parse_block_identifier(boxd, block_identifier):
    if isinstance(block_identifier, int):
        return parse_block_identifier_int(boxd, block_identifier)
        ...
        return boxd.eth.getBlock(block_identifier)['number']
    else:
        raise BlockNumberOutofRange


def parse_block_identifier_int(boxd, block_identifier_int):
    if block_identifier_int >= 0:
        block_num = block_identifier_int
    else:
        last_block = boxd.eth.getBlock('latest')['number']
...
def build_transaction_for_function(
    ...
    prepared_transaction = fill_transaction_defaults(boxd, prepared_transaction)

    return prepared_transaction


def find_functions_by_identifier(contract_abi, boxd, address, callable_check):
